...
const tocItems: TocItem[] = [
  { id: "manifesto", label: "Manifesto", icon: BookOpen, keywords: ["filosofia", "capturar", "organizar", "refletir", "agnóstico", "metodologia", "open source", "gratuito", "pilares"], content: "mindmate nasceu premissa simples mente merece parceiro não prisão metodológica três pilares capturar sem fricção pensamento registro segundos linguagem natural tokens inteligentes organizar sem rigidez projetos tarefas hábitos notas convivem hierarquias artificiais refletir com propósito rituais diários journaling transformam dados autoconhecimento agnósticos metodologia gtd pomodoro bullet journal zettelkasten 100% gratuito open source produtividade bem-estar acessíveis" },
  { id: "arquitetura", label: "Arquitetura Atom", icon: Atom, keywords: ["single table", "items", "type", "project_id", "parent_id", "milestone", "weight", "progresso", "state machine", "draft", "active", "hierarquia"], content: "single table design tabela items type project_id parent_id milestone weight progress_mode auto manual milestone state machine draft active paused completed archived hierarquia plana recurrence_rule rrule completion_log ritual_slot tags módulo order_index checklist due_date deadline" },
  { id: "funcionalidades", label: "Funcionalidades", icon: Rocket, keywords: ["parsing", "token", "inbox", "dashboard", "projeto", "calendário", "ritual", "journal", "lista", "recorrência", "analytics", "hábito", "tag", "módulo", "@hoje", "@amanha", "rrule", "heatmap", "streak"], content: "parsing engine tokens inbox dashboard projeto calendário ritual journal lista recorrência analytics hábito tag módulo @hoje @amanha rrule heatmap streak captura rápida drag-and-drop milestones compartilhamento push notifications offline" },
  // ── Super Manual: Telas Detalhadas ──
  { id: "tela-dashboard", label: "📊 Dashboard", icon: LayoutDashboard, keywords: ["home", "focus", "today", "ritual banner", "sugestões", "confetti", "empty state"], content: "dashboard tela inicial login visão consolidada dia focus block itens marcados #focus completar confetti today list due_date hoje overdue ordenados prioridade ritual banner aurora zênite crepúsculo slot ativo hábitos completion_log streak badge heatmap smart suggestions sugestões inteligentes heurísticas tarefas atrasadas empty state ilustração zen" },
  { id: "tela-inbox", label: "📥 Inbox", icon: Inbox, keywords: ["captura", "parsing", "macropicker", "processar", "promover", "gtd"], content: "inbox captura rápida parsing engine linguagem natural auto-tag #inbox swipe macropicker fluxo inbox→projeto filtros skeleton loading validação zod tokens @hoje @amanha #tag ~manha tipo:task prioridade processar promover gtd" },
  { id: "tela-projetos", label: "📁 Projetos", icon: FolderKanban, keywords: ["workarea", "milestones", "notas", "atividade", "status", "compartilhar", "fab", "drag"], content: "projetos coração organizacional criação status lifecycle draft active paused completed archived 4 abas workarea drag-and-drop milestones peso 3x weight notes activity feed progress modes auto milestone manual compartilhamento roles owner editor viewer convites membros projectfab settings invite code" },
  { id: "tela-listas", label: "📋 Listas", icon: ListChecks, keywords: ["keep", "checklist", "cores", "duplicar", "quick add"], content: "listas cards estilo keep cores personalizáveis checklist integrado duplicação quick-add modal progresso visual itens completados percentual cor fundo" },
  { id: "tela-calendario", label: "📅 Calendário", icon: Calendar, keywords: ["mês", "semana", "drag", "overdue", "day detail", "filtro"], content: "calendário views mês semana drag-and-drop entre dias droppable daycell filtros tipo módulo seção overdue daydetailsheet tarefas hábitos notas data due_date deadline" },
  { id: "tela-journal", label: "📔 Diário", icon: BookMarked, keywords: ["reflexão", "composer", "prompts", "exportar", "markdown", "feed"], content: "diário journal composer prompts reflexivos feed cronológico filtros exportação markdown integration ritual slots reflexão autoconhecimento diário escrita" },
  { id: "tela-rituais", label: "🌅 Rituais", icon: Sun, keywords: ["aurora", "zênite", "crepúsculo", "streak", "heatmap", "confetti", "check-in"], content: "rituais aurora zênite crepúsculo manhã meio-dia noite check-in visual confetti streak tracking completion_log heatmap hábitos recorrentes slot horário 05h-12h 12h-18h 18h-05h" },
  { id: "tela-analytics", label: "📈 Analytics", icon: BarChart3, keywords: ["resumo", "gráfico", "tendência", "weekly summary", "ai"], content: "analytics estatísticas weekly summary ai gráficos tipo módulo streak badges heatmap atividade tendências semana produtividade recharts" },
  // ── Super Manual: Seções Complementares ──
  { id: "modulos-detalhe", label: "🧩 Módulos", icon: Dumbbell, keywords: ["work", "body", "mind", "family", "geral", "#mod_", "herança", "badge"], content: "módulos work body mind family geral #mod_work #mod_body #mod_mind #mod_family herança macropicker modulebadge visual cor ícone briefcase dumbbell brain users" },
  { id: "command-palette", label: "⌨️ Command Palette", icon: Keyboard, keywords: ["⌘k", "busca global", "navegação", "atalho"], content: "command palette ⌘k cmd+k busca global filtros navegação atalhos pesquisar título tags todos tipos itens agrupados categoria navegação projetos listas itens tags roteamento inteligente" },
  { id: "notificacoes", label: "🔔 Notificações", icon: Bell, keywords: ["push", "reminder", "due date", "atividade"], content: "notificações push configuração due-date reminders atividade projetos compartilhados browser notifications service worker permissões lembrete prazo vencimento" },
  { id: "backup-cache", label: "💾 Backup & Cache", icon: Database, keywords: ["offline", "sync", "indexeddb", "export", "rls", "segurança"], content: "backup cache offline sync indexeddb export rls segurança cache local armazenados localmente acesso offline sync automático operações pendentes fifo sync status sincronizado pendente exportação journal markdown error logs csv row-level security dados protegidos open source fork" },
  { id: "onboarding", label: "🚀 Onboarding", icon: UserPlus, keywords: ["welcome", "tour", "checklist", "primeiro uso", "boas-vindas", "gamificação"], content: "onboarding primeiro uso welcome modal boas-vindas 6 slides tour overlay visita guiada 5 passos checklist gamificada primeiros passos missões progresso confetti persistência onboarding_progress multi-dispositivo has_completed_welcome has_completed_tour checklist_progress applayout autenticados" },
  // ── Seções originais ──
  { id: "playground", label: "Token Playground", icon: Play, keywords: ["parsing", "testar", "demo", "interativo", "token", "ao vivo"], content: "token playground ao vivo parsing engine teste interativo digitar texto tokens extraídos tempo real título limpo tipo due date ritual slot tags módulo" },
  { id: "rituais-demo", label: "Rituais Demo", icon: Sun, keywords: ["aurora", "zênite", "crepúsculo", "slot", "manhã", "noite"], content: "rituais demo aurora zênite crepúsculo slot manhã meio-dia noite interativo visualizar hábitos ritual" },
  { id: "casos-de-uso", label: "Casos de Uso", icon: Target, keywords: ["estudante", "freelancer", "equipe", "bem-estar", "cenário"], content: "casos uso estudante freelancer equipe bem-estar cenário universitário projetos semestre freelancer clientes família rotina saúde exercício meditação" },
  { id: "dicas", label: "Dicas & Truques", icon: Lightbulb, keywords: ["avançado", "macro", "workflow", "combinação", "produtividade"], content: "dicas truques avançado macro workflow combinação produtividade tokens combinados atalhos power user fluxo eficiente" },
  { id: "como-usar", label: "Como Usar", icon: ArrowRight, keywords: ["conta", "captura", "fluxo", "passo", "novos usuários", "guia"], content: "como usar conta captura fluxo passo novos usuários guia criar conta primeira tarefa inbox processar organizar projetos rituais diário" },
  { id: "atalhos", label: "Atalhos de Teclado", icon: Keyboard, keywords: ["shortcut", "⌘", "ctrl", "command palette", "teclado", "navegação"], content: "atalhos teclado shortcut ⌘ ctrl command palette navegação ⌘k ⌘n escape ctrl+enter tab shift+tab" },
  { id: "colaboracao", label: "Colaboração", icon: Users, keywords: ["owner", "editor", "viewer", "convite", "role", "compartilhar", "activity", "membro"], content: "colaboração owner editor viewer convite role compartilhar activity membro projeto compartilhado código convite link roles permissões remover membro alterar papel" },
  { id: "pwa", label: "PWA & Offline", icon: Wifi, keywords: ["instalação", "offline", "push", "notificação", "sync", "ios", "android", "desktop"], content: "pwa progressive web app offline push notificação sync ios android desktop instalação service worker cache indexeddb tela cheia splash screen ícone" },
  { id: "glossario", label: "Glossário", icon: BookText, keywords: ["atom", "token", "ritual slot", "milestone", "weight", "rrule", "módulo", "parsing", "termos"], content: "glossário atom token ritual slot milestone weight rrule módulo parsing termos definição vocabulário conceitos" },
  { id: "diagrama", label: "Arquitetura Visual", icon: Layers, keywords: ["diagrama", "ascii", "single table", "hierarquia", "fluxo"], content: "diagrama ascii single table hierarquia fluxo arquitetura visual estrutura banco dados relacionamento" },
  { id: "changelog", label: "Changelog", icon: History, keywords: ["versão", "atualização", "mudança", "release", "alpha"], content: "changelog versão atualização mudança release alpha histórico novidades correções melhorias" },
  { id: "faq", label: "FAQ", icon: HelpCircle, keywords: ["gratuito", "seguro", "exportar", "dados", "tags", "dispositivos", "conexão"], content: "faq perguntas frequentes gratuito seguro exportar dados tags dispositivos conexão privacidade offline funciona sem internet open source código aberto" },
];
